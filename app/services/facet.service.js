...
angular.module('bib.services').factory('facetService', function($filter) {
  ...
  var facets = [
    ...
    },
    {
      ...
      parser: function(affiliations) {
        return affiliations.map(function(a) {
          return a.name;
        });
      }
    },
        ...
        .map(
          _.spread(
            function(label, count) {
              var enabled = Boolean(
                _.find(this.enabledItems, { facetId: facetId, label: label })
              );
              return {
                label: label,
                count: count,
